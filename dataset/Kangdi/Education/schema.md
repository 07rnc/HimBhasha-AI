...
This domain uses the standard HIMCorpus Schema. The table below represents the columns and values specific to the `education` domain.

| Column | Constained Value / Format | Description |
| ------ | ------------------------- | ----------- |
| `id` | `kng_edu_[seq]` | Unique sequence ID |
| `domain` | `education` | Category tag |
| `intent` | Must be one of: `classroom`, `subject`, `instruction`, `question`, `praise` | Conversational purpose |
