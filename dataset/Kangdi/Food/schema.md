...
This domain uses the standard HIMCorpus Schema. The table below represents the columns and values specific to the `food` domain.

| Column | Constained Value / Format | Description |
| ------ | ------------------------- | ----------- |
| `id` | `kng_food_[seq]` | Unique sequence ID |
| `domain` | `food` | Category tag |
| `intent` | Must be one of: `dish`, `ingredient`, `taste`, `cooking`, `meal` | Conversational purpose |
